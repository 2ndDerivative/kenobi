...
use windows::Win32::{
    Foundation::{
        FILETIME, SEC_E_INCOMPLETE_MESSAGE, SEC_E_INSUFFICIENT_MEMORY, SEC_E_INTERNAL_ERROR,
        SEC_E_INVALID_HANDLE, SEC_E_INVALID_TOKEN, SEC_E_LOGON_DENIED,
        SEC_E_NO_AUTHENTICATING_AUTHORITY, SEC_E_NO_CREDENTIALS, SEC_E_OK,
        SEC_E_UNSUPPORTED_FUNCTION,
    },
    Security::{
        Authentication::Identity::{
            AcceptSecurityContext, DeleteSecurityContext, ASC_REQ_CONFIDENTIALITY,
            ASC_REQ_MUTUAL_AUTH, SECURITY_NATIVE_DREP,
        },
...
impl ContextHandle {
    pub fn step(
        ...
        let old_context: Option<*const SecHandle> =
            context.as_ref().map(|x| std::ptr::from_ref(x.as_ref()));
        ...
        let is_done = match res {
            ...
            SEC_E_INCOMPLETE_MESSAGE => {
                return Err(StepError::IncompleteMessage)
            },
            ...
            SEC_E_INVALID_TOKEN => {
                return Err(StepError::InvalidToken)
            },
            SEC_E_LOGON_DENIED => {
                return Err(StepError::LogonDenied)
            },
